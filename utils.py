...
def MMD_batch2(Fea, len_s, Fea_org, sigma, sigma0=0.1, epsilon = 10**(-10), is_smooth=True, is_var_computed=True, use_1sample_U=True, coeff_xy=2):
	...
	K_Ix = torch.eye(nx).cuda() # Create an identity matrix
	K_Iy = torch.eye(ny).cuda() 

	if is_smooth:
		Kx = (1-epsilon) * torch.exp(-(Dxx / sigma0)**L -Dxx_org / sigma) + epsilon * torch.exp(-Dxx_org / sigma) # 400,400
		Ky = (1-epsilon) * torch.exp(-(Dyy / sigma0)**L -Dyy_org / sigma) + epsilon * torch.exp(-Dyy_org / sigma) # 2000,1 value 1
...
def TST_C2ST_L(pred_C2ST, N_per, N1, alpha):
	...
	for r in range(N_per):
		...
		ind_Y = ind[N1:]
		# print(indx)
		STAT_vector[r] = abs(pred_C2ST[ind_X,1].type(torch.FloatTensor).mean() - pred_C2ST[ind_Y,1].type(torch.FloatTensor).mean())
