...
from utils_MMD import MMDu, MMD_batch2, TST_MMD_u, TST_MMD_u_3S, TST_MMD_u_3S_Baseline, TST_MMD_u_3S_Permutation, TST_MMD_u_3S_Permutation_Kernel
...
def plot_mi(clean, adv):
    ...
    print("auroc: {:.4f}; ".format(ap) + "; ".join(
        ["TPR: {:.4f} @ FPR={:.4f}".format(v, k) for k, v in accs.items()]) + "  {}-{}".format(len(mi_nat),
                                                                                               len(mi_svhn)))
    return ap
...
def sample_from_model(texts, min_words=55, prompt_tokens=30):
    ...
    if args.openai_model:
        ...
        while (min(len(x.split()) for x in decoded)) < min_words:
            ...
            decoded = base_tokenizer.batch_decode(outputs[:, prompt_tokens:],
                                                  skip_special_tokens=True)  # remove the first 30 tokens
            tries += 1
...
def generate_samples(raw_data, batch_size, dataset=None):
    ...
    for batch in range(len(raw_data) // batch_size):
        ...
        sampled_text = sample_from_model(original_text, min_words=30 if dataset in ['pubmed'] else 50,
...
def load_HC3(data_o, long_train_threshold_low=150, long_train_threshold_high=512):
    ...
    long_train_real = [x for x, y in zip(long_train_real, tokenized_data["input_ids"]) if len(y) <= long_train_threshold_high]
    tokenized_data = preproc_tokenizer(long_train_generated)
    long_train_generated = [x for x, y in zip(long_train_generated, tokenized_data["input_ids"]) if len(y) <= long_train_threshold_high]
...
def load_base_model_and_tokenizer(name):
    ...
    if name in ['roberta-base', 'roberta-base-openai-detector', 'Hello-SimpleAI/chatgpt-detector-roberta']:
...
def fea_get(texts_ls, max_length=300, print_fea_dim=True):
    with torch.no_grad():
        ...
        for batch in range(math.ceil(len(texts_ls) / batch_size)):
            ...
            inputs = base_tokenizer(batch_real, padding='max_length', truncation=True, max_length=max_length,
                                    return_tensors="pt").to(DEVICE)

            if args.base_model_name not in [
                'roberta-base', 'roberta-base-openai-detector',
                'Hello-SimpleAI/chatgpt-detector-roberta', 'tiiuae/falcon-rw-1b'
            ]:
                if args.mask_flag and not args.test_flag:
                    input_ids = inputs['input_ids']
                    mask_ratio = 0.15
                    mask_positions = (torch.rand_like(input_ids.float()) < mask_ratio) & (
                            input_ids != base_tokenizer.cls_token_id) & (input_ids != base_tokenizer.sep_token_id)
                ...
                if outputs.hidden_states is not None:
                    ...
                    raise ValueError(f"The model {args.base_model_name} does not have hidden states")
            else:
...
def process_p_values_and_labels_odd(answer_labels, results_list):
    ...
    # extension tag
    expanded_labels = np.concatenate([np.repeat(label, count) for label, count in zip(answer_labels, repeat_counts)])
    ...
    # Print results
    print("ALL_AUROC: {:.4f}; ALL_AVG_FPR: {:.4f}; ALL_STD_FPR: {:.4f}; ALL_AVG_TPR: {:.4f}; ALL_STD_TPR: {:.4f}".format(
...
def process_p_values_and_labels(answer_labels, results_list):
    ...
    for i in range(len(answer_labels)):
        ...
        for j in range(len(answer_labels)):
            if answer_labels[j] != current_label and j not in used_indices:
                ...
                # Calculate AUROC
                auroc = round(metrics.roc_auc_score(combined_labels, combined_p_values), 6)
                fpr, tpr, thresholds = metrics.roc_curve(combined_labels, combined_p_values)
    ...
    interpolated_fpr = [np.interp(np.linspace(0, 1, fixed_length), np.linspace(0, 1, len(fpr)), fpr) for fpr in
                        fpr_list]
    interpolated_tpr = [np.interp(np.linspace(0, 1, fixed_length), np.linspace(0, 1, len(tpr)), tpr) for tpr in
                        tpr_list]
    ...
    # Print results
    print("AVG_AUROC: {:.4f}; STD_AUROC: {}; AVG_FPR: {:.4f}; STD_FPR: {}; AVG_TPR: {:.4f}; STD_TPR: {}".format(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--target_datasets', type=str, nargs='+', default="HC3",
    ...
    parser.add_argument('--base_model_name', type=str, default="roberta-base-openai-detector",
                        help='gpt2-medium|roberta-base|roberta-base-openai-detector|tiiuae/falcon-rw-1b')
    parser.add_argument('--text_generated_model_name', type=str, nargs='+', default="gpt2",
    ...
    parser.add_argument('--metric', type=str, choices=["auroc", "power"], default="power", help="auroc|power")
    ...
    parser.add_argument('--relative_test_reference_mode', type=str, choices=["random", "all"], default="random", help="random|all")
    parser.add_argument('--relative_test_mode', type=str, choices=["normal", "permutation"], default="normal", help="normal|permutation")
    parser.add_argument('--relative_test_two_sample_mode', type=str, choices=["normal", "permutation"], default="normal", help="normal|permutation")

    ## Add extra arguments for the command line to determine the plain text to be used for testing the model
    parser.add_argument('--test_dataset', type=str, default=None, help='HC3|SQuAD1|TruthfulQA|CCNews')
    parser.add_argument('--test_dataset_answer', type=str, default="ChatGPT", help='ChatGPT|ChatGPT-turbo|GPT4|StableLM|BloomZ|ChatGLM|Dolly|Human')
    ...
    parser.add_argument('--test_dataset_attack', type=str, default='none', help='any|none')
    parser.add_argument('--raid_split', type=str, default='train', help='train|test|extra', choices=['train', 'test', 'extra'])
    ...
    assert args.test_dataset == None or (args.test_text is None and args.test_text_file is None), "test_dataset must not be set or None if test_text or test_text_file is not None"
    ...
    assert not args.test_text_split or (args.test_text_n_samples is None and args.test_text_n_sample_rounds is None), "test_text_n_samples and test_text_n_sample_rounds is only allowed if test_text_split is set to False"
    ...
    assert args.test_dataset_attack == 'none' or args.test_dataset in ["RAID", "Beemo", "DetectRL"], "test_dataset_attack is only allowed for RAID, Beemo and DetectRL dataset"

    assert args.test_dataset_answer != "Human" or args.test_dataset_answer_mix_ratio is None, "test_dataset_answer must mix with other answers than Human"

    if args.faster and args.target_datasets != ['HC3']:
        print("Warning: faster flag is made mainly for HC3 dataset, it may not work well or cause unexpected behavior for other datasets")
    ...
    if args.test_dataset_answer_mix_ratio is not None:
        ...
        if args.test_dataset != None:
            answer_labels = [0] * args.trial_num if args.test_dataset_answer == "Human" else [1] * args.trial_num
        else:
            answer_labels = []
    ...
    for seed in range(990, 990 + args.trial_num):
        ...
        if args.test_dataset_answer_mix_ratio is not None:
            ...
            args.test_dataset_answer = "Human" if current_trial_label == 0 else test_data_answer
        current_trial += 1
        current_time_str = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        ...
        if not args.baselines_only and not args.random_fills:
            ...
            mask_model = transformers.AutoModelForSeq2SeqLM.from_pretrained(model_path_dit[mask_filling_model_name],
        ...
        mask_tokenizer = transformers.AutoTokenizer.from_pretrained(model_path_dit[mask_filling_model_name],
        ...
        for target_dataset in args.target_datasets:
            ...
            if target_dataset in ['xsum', 'squad', 'writing']:
                ...
                if target_dataset in custom_datasets.DATASETS:
                    ...
                    train_real_o = datasets.load_dataset(target_dataset, split='train', cache_dir=cache_dir)[
                        dataset_key]

            elif target_dataset in ['HC3', 'TruthfulQA', 'SQuAD1']:
                data_o = dataset_loader.load(target_dataset, args.cache_dir, train_ratio=0.8 if not args.faster else 1.0)  # train 38154;   test 9538
                train_real_o = [text for text, label in zip(data_o['train']['text'], data_o['train']['label']) if
                                label == 0]  # 19077[text,text]
            elif target_dataset in ['RAID']:
                data_o = dataset_loader.load(target_dataset + "_LLMs", args.cache_dir, LLM_name='train', raid_split='train',
                                             train_ratio=0.1)
                train_real_o = [text for text, label in zip(data_o['train']['text'], data_o['train']['label']) if
                                label == 0]  # 19077[text,text]
            else:
            ...
            if args.test_dataset is not None:
                print(f'Loading test dataset {args.test_dataset}, with answers from {args.test_dataset_answer}...')
                if args.test_dataset in ['CCNews']:
                    assert args.test_dataset_answer in ['Human'], "Unsupported test dataset answer"
                    test_o = dataset_loader.load(args.test_dataset, args.cache_dir, train_ratio=0.1)
                    test_o = [text for text, label in zip(test_o['test']['text'], test_o['test']['label']) if
                              label == 0]
                elif args.test_dataset in ['HC3', 'TruthfulQA', 'TruthfulQA_adv1', 'TruthfulQA_adv2', 'SQuAD1', 'RAID', 'Beemo', 'DetectRL']:
                    if args.test_dataset_answer == "ChatGPT" and args.test_dataset not in ['RAID', 'Beemo', 'DetectRL']:
                        test_o = dataset_loader.load(args.test_dataset, args.cache_dir, train_ratio=0.1)
                        test_o = [text for text, label in zip(test_o['test']['text'], test_o['test']['label']) if
                                        label == 1] ## Label = 1: chatgpt_answer, Label = 0: human_answer
                    elif args.test_dataset_answer == "Human" and args.test_dataset == 'HC3':
                        test_o = dataset_loader.load(args.test_dataset, args.cache_dir, train_ratio=0.1)
                        test_o = [text for text, label in zip(test_o['test']['text'], test_o['test']['label']) if
                                  label == 0]  ## Label = 1: llm_answer, Label = 0: human_answer
                    else:
                        assert args.test_dataset in ['TruthfulQA', 'SQuAD1', 'RAID', 'Beemo', 'DetectRL'], "Unsupported test dataset for given test dataset LLM"
                        if args.test_dataset_answer.lower() == "human" and args.test_dataset in ['DetectRL']:
                            test_o = dataset_loader.load(args.test_dataset + '_LLMs', args.cache_dir, # add attack for DetectRL even when test_dataset_answer is human
                                                         LLM_name="human", raid_split=args.raid_split, attack=args.test_dataset_attack, train_ratio=0.1, max_tokens=args.test_text_n_sample_tokens)
                            test_o = [text for text, label in zip(test_o['test']['text'], test_o['test']['label']) if
                                      label == 0]  ## Label = 1: llm_answer, Label = 0: human_answer
                        elif args.test_dataset_answer.lower() == "human" and args.test_dataset in ['RAID', 'Beemo']:
                            test_o = dataset_loader.load(args.test_dataset + '_LLMs', args.cache_dir,
                                                         LLM_name="human", raid_split=args.raid_split, train_ratio=0.1, max_tokens=args.test_text_n_sample_tokens)
                            test_o = [text for text, label in zip(test_o['test']['text'], test_o['test']['label']) if
                                      label == 0]  ## Label = 1: llm_answer, Label = 0: human_answer
                        elif args.test_dataset_answer.lower() == "machine" and args.test_dataset in ['RAID', 'Beemo', 'DetectRL']:
                            test_o = dataset_loader.load(args.test_dataset + '_LLMs', args.cache_dir,
                                                         LLM_name="machine", raid_split=args.raid_split, attack=args.test_dataset_attack, train_ratio=0.1, max_tokens=args.test_text_n_sample_tokens)
                            test_o = [text for text, label in zip(test_o['test']['text'], test_o['test']['label']) if
                                      label == 1]  ## Label = 1: llm_answer, Label = 0: human_answer
                        elif args.test_dataset_answer == "Human":
                            test_o = dataset_loader.load(args.test_dataset + '_LLMs', args.cache_dir,
                                                         LLM_name="ChatGPT", train_ratio=0.1)
                            test_o = [text for text, label in zip(test_o['test']['text'], test_o['test']['label']) if
                                      label == 0]  ## Label = 1: llm_answer, Label = 0: human_answer
                        else:
                            test_o = dataset_loader.load(args.test_dataset+'_LLMs', args.cache_dir, LLM_name=args.test_dataset_answer, train_ratio=0.1)
                            test_o = [text for text, label in zip(test_o['test']['text'], test_o['test']['label']) if
                                            label == 1] ## Label = 1: llm_answer, Label = 0: human_answer
                else:
            ...
            for text_generated_model_name in args.text_generated_model_name:
                ...
                if args.two_sample_test:
                    ...
                    real_sent_token = [nltk.sent_tokenize(text)[1:-1] if target_dataset in ['HC3', 'RAID'] else nltk.sent_tokenize(text) for text in real]
                    generated_sent_token = [nltk.sent_tokenize(text)[1:-1] if target_dataset in ['HC3', 'RAID'] else nltk.sent_tokenize(text) for text in generated]
                    ...
                    if not args.faster or current_trial == 1:  ## If not faster mode or first trial
                        ...
                        real_data_temp = [[sentence for sentence in sublist if len(sentence.split()) >= 5] for sublist in
                    ...
                    if target_dataset in ['HC3', 'RAID']:
                        ...
                        sentence_num = 2
                    if not args.faster or current_trial == 1:  ## If not faster mode or first trial
                        real_data_temp_seletced = [pa_ls for pa_ls in real_data_temp if len(pa_ls) >= sentence_num]
                        generated_data_temp_seletced = [pa_ls for pa_ls in generated_data_temp if len(pa_ls) >= sentence_num]
                    ...
                    if not args.test_text_split and len(test_data_temp_seletced) == 1:
                        if not args.test_text_n_samples or args.test_text_n_samples == -1:
                            args.test_text_n_samples = len(test_data_temp_seletced[0])
                        args.test_text_n_samples = max(min(args.test_text_n_samples, len(test_data_temp_seletced[0])),
                                                       2)
                        test_data_temp_seletced[0] = test_data_temp_seletced[0][
                                                     :args.test_text_n_samples] if args.test_text_n_samples else \
                        test_data_temp_seletced[0]
                    else:
                        if not args.test_text_n_samples or args.test_text_n_samples == -1:
                            args.test_text_n_samples = len(test_data_temp_seletced)
                        args.test_text_n_samples = max(min(args.test_text_n_samples, len(test_data_temp_seletced)), 2)
                        test_data_temp_seletced = test_data_temp_seletced[
                                                  :args.test_text_n_samples] if args.test_text_n_samples else test_data_temp_seletced

                    ## Cut the test data to the required number of tokens
                    if not args.test_text_split and len(test_data_temp_seletced) == 1:
                        if args.test_text_n_sample_tokens is not None and args.test_text_n_sample_tokens != -1:
                            total_tokens = sum([len(sen.split()) for sen in test_data_temp_seletced[0]])
                            args.test_text_n_sample_tokens = max(min(args.test_text_n_sample_tokens, total_tokens), 2)
                            total_tokens = 0
                            for i in range(len(test_data_temp_seletced[0])):
                                total_tokens += len(test_data_temp_seletced[0][i].split())
                                if total_tokens >= args.test_text_n_sample_tokens:
                                    token_diff = total_tokens - args.test_text_n_sample_tokens
                                    test_data_temp_seletced[0] = test_data_temp_seletced[0][:i + 1]
                                    if token_diff > 0:
                                        test_data_temp_seletced[0][-1] = ' '.join(
                                            test_data_temp_seletced[0][-1].split()[:token_diff])
                    ...
                    if args.test_text is not None or args.test_text_file is not None or args.test_dataset is not None:
                        if args.relative_test_extra_n_samples == -1:
                            ...
                            if args.test_text_split:
                                ...
                                for i in range(len_data):
                                    count += min(len(real_data_temp_seletced[i]), len(generated_data_temp_seletced[i]))
                                    if count >= len(test_data_temp_seletced[0]):
                                        if args.relative_test_extra_n_samples == -2:
                                            args.relative_test_extra_n_samples = i
                                        test_lenth = i + args.relative_test_extra_n_samples + 1
                                        break
                                if count < len(test_data_temp_seletced[0]) or test_lenth > len_data - args.val_num - EXTRA_SAMPLES:
                                    test_lenth = len_data - args.val_num - EXTRA_SAMPLES
                    ## Check if the length of the remaining data is enough for the required number of sentences
                    assert test_lenth > 0 and len_data >= args.val_num + test_lenth + EXTRA_SAMPLES, print(
                ...
                text_single_real = real_data_temp_seletced[args.val_num + test_lenth: args.val_num + test_lenth + EXTRA_SINGLE_SAMPLES]
                text_single_generated = generated_data_temp_seletced[
                                        args.val_num + test_lenth: args.val_num + test_lenth + EXTRA_SINGLE_SAMPLES]
                text_reference = real_data_temp_seletced[
                                 args.val_num + test_lenth + EXTRA_SINGLE_SAMPLES: args.val_num + test_lenth + EXTRA_SAMPLES]
                ...
                fea_test_ori = [pa for pa in test_data_temp_seletced] if len(test_data_temp_seletced) > 0 else []
            ...
            with torch.no_grad():
                ...
                if not test_flag:
                    ...
                    if auroc_value > auroc_value_best:
                        ...
                        print("Save the best model: auroc_value_best=", auroc_value_best)
            # torch.save(state, model_path + '/'+ meta_save_model_flag +'last_ckpt.pth')
            return auroc_value


        def two_sample_test(epoch, test_lenth=2000, fea_real_ls=fea_real, fea_generated_ls=fea_generated, fea_test_ls=fea_generated,
                            meta_save_model_flag='', test_flag=args.test_flag, N=100, relative_test=args.relative_test, return_p_and_stat=False, baseline=False, relative_test_baseline=False, relative_test_permutation=False, relative_test_baseline_simple=False):
            global power_best
            model_path = f'./{PATH_exper}/HC3-{args.base_model_name}/{id}'
            ## Set the model to evaluation mode
            net.eval()
            with torch.no_grad():
                ...
                def mmd_two_sample(fea_real_ls, fea_generated_ls, N=100):
                    ...
                    for i in range(len(fea_real_ls)):
                        ...
                        count_u = 0
                        for _ in range(N):
                            h_u, threshold_u, mmd_value_u = TST_MMD_u(torch.cat([final_x, final_y], dim=0), N_per,
                                                                      final_x.shape[0],
                                                                      torch.cat([fea_x_ori, fea_y_ori], dim=0).view(
                                                                          fea_x_ori.shape[0] + fea_y_ori.shape[0], -1),
                                                                      sigma, sigma0_u, ep, alpha)
                            count_u = count_u + h_u
                ...
                def mmd_two_sample_baseline_permutation(fea_test_ls, fea_real_ls, fea_generated_ls, N=100):
                    if not args.test_text_split:
                        ## Concatenate the hidden states of the real and generated data
                    ...
                    for i in range(len(fea_test_ls)):
                        ...
                        for j in range(len(fea_real_ls)):
                            ...
                            count_u = 0
                            for _ in range(N):
                                final_real = torch.cat([final_x, final_y], dim=0)[:((final_x.shape[0] + final_y.shape[0])//2) * 2]
                                final_generated = torch.cat([final_x, final_z], dim=0)[:((final_x.shape[0] + final_z.shape[0])//2) * 2]
                                h_u_real, p_real, mmd_value_u_real = TST_MMD_u(final_real, N_per,
                                                                               (final_real.shape[0]//2),
                                                                          final_real.view(
                                                                              final_real.shape[0], -1),
                                                                          sigma, sigma0_u, ep, args.relative_test_alpha)
                                ## interchanging the real and generated data
                                h_u_generated, p_generated, mmd_value_u_generated = TST_MMD_u(final_generated, N_per,
                ...
                def mmd_two_sample_baseline(fea_test_ls, fea_real_ls, fea_generated_ls, N=100):
                    ## Concatenate the hidden states of the real and generated data
                    ...
                    for _ in range(max(args.test_text_n_sample_rounds, 1)):
                        ...
                        if not args.test_text_split and len(fea_test_ls) == 1 and args.test_text_n_sample_rounds:
                            ## Randomly sample n samples from the reference data tensor
                            fea_test_ls[0] = fea_test_ls[0][torch.randperm(len(fea_test_ls[0]))[:args.test_text_n_samples]]
                        for i in range(len(fea_test_ls)):
                            fea_x_ori = fea_test_ls[i].to('cuda')
                            fea_y_ori = fea_real_ls[torch.randperm(len(fea_real_ls))]
                            fea_z_ori = fea_generated_ls[torch.randperm(len(fea_generated_ls))]

                            min_len = min(len(fea_x_ori), len(fea_y_ori), len(fea_z_ori))
                            ...
                            count_u = 0
                            for _ in range(N):
                                h_u_real, p_real, mmd_value_u_real = TST_MMD_u(torch.cat([final_y, final_x], dim=0), N_per,
                                                                          final_x.shape[0],
                                                                          torch.cat([fea_y_ori, fea_x_ori], dim=0).view(
                                                                              fea_y_ori.shape[0] + fea_x_ori.shape[0], -1),
                                                                          sigma, sigma0_u, ep, args.relative_test_alpha)
                                ## interchanging the real and generated data
                                h_u_generated, p_generated, mmd_value_u_generated = TST_MMD_u(torch.cat([final_x, final_z], dim=0), N_per,
                                                                 final_x.shape[0],
                ...
                def mmd_three_sample_base(fea_test_ls, fea_real_ls, fea_generated_ls, use_all_references=False, N=10, TST_MMD=TST_MMD_u_3S, TST_MMD_kwargs={}, use_meta_model=False, cut_same_length=True):
                    ## y = real, z = generated
                    ...
                    for _ in range(max(args.test_text_n_sample_rounds, 1)):
                        ...
                        ## If the reference data is not splitted, then randomly sample n samples from the reference data
                        if not args.test_text_split and len(fea_test_ls_ori) == 1 and args.test_text_n_sample_rounds:
                            ## Randomly sample n samples from the reference data tensor
                            fea_test_ls[0] = fea_test_ls_ori[0][torch.randperm(len(fea_test_ls_ori[0]))[:args.test_text_n_samples]]

                        for i in range(len(fea_test_ls)):
                            fea_test_ori = fea_test_ls[i].to('cuda')
                            min_len = min(len(fea_real_ls), len(fea_generated_ls), len(fea_test_ori))
                            if cut_same_length:
                            ...
                            if use_all_references:
                                max_len = len(fea_real_ls) - len(fea_test_ori) if len(fea_real_ls) > len(fea_test_ori) else len(fea_real_ls)
                                step_len = min(len(fea_real_ls), len(fea_generated_ls), len(fea_test_ori))
                                if not cut_same_length:
                                    max_len = len(fea_real_ls)
                                    step_len = min(len(fea_generated_ls), len(fea_real_ls))
                            else:
                                max_len = N
                                step_len = 1
                            j = 0
                            while j < max_len:
                                if use_all_references:
                                    ...
                                    fea_generated_ori = fea_generated_ls[j:j + step_len]
                                    final_x = net(fea_test_ori)
                                else:
                                    if cut_same_length:
                                        fea_real_ori = fea_real_ls[torch.randperm(len(fea_real_ls))[:min_len]]
                                        fea_generated_ori = fea_generated_ls[torch.randperm(len(fea_generated_ls))[:min_len]]
                                    else:
                                        fea_real_ori = fea_real_ls[torch.randperm(len(fea_real_ls))]
                                        fea_generated_ori = fea_generated_ls[torch.randperm(len(fea_generated_ls))]

                                final_y = net(fea_real_ori)
                                final_z = net(fea_generated_ori)

                                if use_meta_model:
                                    h_u, p_value, t, *rest = TST_MMD(final_x, final_y, final_z, fea_test_ori.view(fea_test_ori.shape[0], -1), fea_real_ori.view(fea_real_ori.shape[0], -1), fea_generated_ori.view(fea_generated_ori.shape[0], -1), **TST_MMD_kwargs)
                                else:
                                    h_u, p_value, t, *rest = TST_MMD(fea_test_ori, fea_real_ori, fea_generated_ori, **TST_MMD_kwargs)
                                ...
                                if use_all_references:
                                    ...
                                    if (j + step_len * 2 > max_len) and (j + step_len < max_len):
                ...
                def mmd_three_sample(fea_test_ls, fea_real_ls, fea_generated_ls, N=10):
                    ...
                    return mmd_three_sample_base(fea_test_ls, fea_real_ls, fea_generated_ls, use_all_references=(args.relative_test_reference_mode == 'all'), N=N, TST_MMD=TST_MMD_u_3S, TST_MMD_kwargs=TST_MMD_kwargs, use_meta_model=True)

                def mmd_three_sample_permutation(fea_test_ls, fea_real_ls, fea_generated_ls, N=10):
                    TST_MMD_kwargs = {
                        'sigma': sigma,
                        'sigma0': sigma0_u,
                        'epsilon': ep,
                    ...
                    return mmd_three_sample_base(fea_test_ls, fea_real_ls, fea_generated_ls, use_all_references=True, N=N, TST_MMD=TST_MMD_u_3S_Permutation_Kernel, TST_MMD_kwargs=TST_MMD_kwargs, use_meta_model=True, cut_same_length=False)

                def mmd_three_sample_baseline(fea_test_ls, fea_real_ls, fea_generated_ls, no_median_heuristic=False, N=10):
                    TST_MMD_kwargs = {
                        # 'sigma': 1.0,
                        'alpha': args.relative_test_alpha,
                        'return_sigma': (not test_flag),
                        'sigma_path': model_path + '/' + meta_save_model_flag + 'relative_test_baseline_sigma.npy',
                         'no_median_heuristic': no_median_heuristic
                    }
                    return mmd_three_sample_base(fea_test_ls, fea_real_ls, fea_generated_ls, use_all_references=(args.relative_test_reference_mode == 'all'), N=N, TST_MMD=TST_MMD_u_3S_Baseline, TST_MMD_kwargs=TST_MMD_kwargs)

                p_value_ls = None
                if relative_test_baseline and not test_flag:
                    test_power_ls, p_value_ls, t_ls, *relative_test_baseline_sigma = mmd_three_sample_baseline(
                        fea_test_ls, fea_real_ls, fea_generated_ls, N=N)
                if baseline:
                    if args.relative_test_two_sample_mode == 'normal':
                        test_power_ls, p_value_ls, t_ls = mmd_two_sample_baseline(fea_test_ls, fea_real_ls, fea_generated_ls, N=N)
                    elif args.relative_test_two_sample_mode == 'permutation':
                        test_power_ls, p_value_ls, t_ls = mmd_two_sample_baseline_permutation(fea_test_ls, fea_real_ls,fea_generated_ls, N=N)
                elif relative_test_baseline and test_flag:
                    test_power_ls, p_value_ls, t_ls = mmd_three_sample_baseline(fea_test_ls, fea_real_ls, fea_generated_ls, N=N)
                elif relative_test_baseline_simple:
                    test_power_ls, p_value_ls, t_ls = mmd_three_sample_baseline(fea_test_ls, fea_real_ls, fea_generated_ls, N=N, no_median_heuristic=True)
                elif relative_test_permutation:
                    ## Get the p_value of the relative test
                    test_power_ls, p_value_ls, t_ls= mmd_three_sample_permutation(fea_test_ls, fea_real_ls, fea_generated_ls, N=N)
                elif relative_test:
                    ## Get the p_value of the relative test
                    test_power_ls, p_value_ls, t_ls= mmd_three_sample(fea_test_ls, fea_real_ls, fea_generated_ls, N=N)
                else:
                    ## Get the test power of the real and generated data pairs
                    test_power_ls, t_ls = mmd_two_sample(fea_real_ls, fea_generated_ls, N=N)

                ## Calculate p_value and/or avg_t
                if p_value_ls:
                    ## Avoid p_value_ls to be 1 or 0
                    p_value_ls_temp = [min(max(p, 1e-08), 1 - 1e-08) for p in p_value_ls]
                ...
                if not test_flag:
                    ...
                    if power > power_best:
                        ...
                        if relative_test_baseline:
                            np.save(model_path + '/' + meta_save_model_flag + 'relative_test_baseline_sigma.npy', relative_test_baseline_sigma)
                            print("Save the best relative test baseline sigma: power_best=", power_best)
                        torch.save(state, model_path + '/' + meta_save_model_flag + 'best_ckpt.pth')
                        print("Save the best model: power_best=", power_best)
            # torch.save(state, model_path + '/'+ meta_save_model_flag +'last_ckpt.pth')
            if relative_test and return_p_and_stat:
                return power, p_value_ls, avg_t
            if (baseline or relative_test_baseline or relative_test_baseline_simple) and return_p_and_stat:
                return power, p_value_ls, avg_t
            return power
        ...
        def train(epoch):
            ...
            net.train()
            for batch in tqdm.tqdm(range(len(fea_train_generated) // train_batch_size), desc="Training for deep MMD"):
                ...
                TEMP = MMDu(outputs, inputs.shape[0], X.view(X.shape[0], -1), sigma, sigma0_u, ep,
                            coeff_xy=args.coeff_xy, is_yy_zero=args.is_yy_zero, is_xx_zero=args.is_xx_zero)
        ...
        if not args.test_flag:
            ...
            for epoch in range(start_epoch, start_epoch + epochs):
                ...
                if (epoch + 1) % 1 == 0:
                    ...
                    ## in this case, we're using the two sample test, so set the test flag of single instance test to True, so we're not saving the model according to the single instance test
                        power = two_sample_test(epoch, fea_real_ls=val_real, fea_generated_ls=val_generated, fea_test_ls=val_generated, N=10, test_flag=False, relative_test_baseline=args.relative_test_baseline)
                        auroc_value_epoch = single_instance_test(epoch, fea_real=val_sing_real, fea_generated=val_sing_generated, test_flag=True)
                    else:
                        power = two_sample_test(epoch, fea_real_ls=val_real, fea_generated_ls=val_generated, fea_test_ls=val_generated, N=10, test_flag=True)
            ...
            for i in range(num_target):
                ## Code for different metrics
                if args.metric == "power":
                    two_sample_test(0, fea_real_ls=fea_real[i * test_lenth:(i + 1) * test_lenth],
                                    fea_generated_ls=fea_generated[i * test_lenth:(i + 1) * test_lenth], fea_test_ls=fea_generated[i * test_lenth:(i + 1) * test_lenth], test_flag=True,
                                    N=10)
                else:
            ...
            if args.test_text is None and args.test_text_file is None and not args.relative_test:
                if args.relative_test:
                    power, p_value, t = two_sample_test(epoch, test_flag=True, return_p_and_stat=True)
                ...
                power_real = two_sample_test(epoch, fea_real_ls=fea_test, test_flag=True, relative_test=False)
                ...
                if args.relative_test:
                    ...
                    power_baseline, p_value_baseline, mmd_baseline = two_sample_test(epoch, fea_test_ls=fea_test, test_flag=True, baseline=True, return_p_and_stat=True, relative_test=False)
                    end_time = time.time()
                    ...
                    if args.relative_test_baseline_simple:
                        ...
                        start_time = time.time()
                        power_simple_relative_baseline, p_value_simple_relative_baseline, t_simple_relative_baseline = two_sample_test(epoch, fea_test_ls=fea_test, test_flag=True, return_p_and_stat=True, relative_test=False, relative_test_baseline_simple=True)
                        end_time = time.time()
                        simple_relative_baseline_time_list.append(np.round(end_time - start_time, 6))
                        simple_relative_baseline_p_value_list.append(np.round(p_value_simple_relative_baseline, 6))
                        simple_relative_baseline_t_stat_list.append(np.round(t_simple_relative_baseline, 6))
                        simple_relative_baseline_power_list.append(np.round(power_simple_relative_baseline, 6))
                    if args.relative_test_baseline:
                        ## Then, run relative test baseline
                        start_time = time.time()
                        power_relative_baseline, p_value_relative_baseline, t_relative_baseline = two_sample_test(epoch, fea_test_ls=fea_test, test_flag=True, relative_test_baseline=True, return_p_and_stat=True, relative_test=False)
                        end_time = time.time()
                        relative_baseline_time_list.append(np.round(end_time - start_time, 6))
                        relative_baseline_p_value_list.append(np.round(p_value_relative_baseline, 6))
                        relative_baseline_t_stat_list.append(np.round(t_relative_baseline, 6))
                        relative_baseline_power_list.append(np.round(power_relative_baseline, 6))
                    if args.binoculars or args.output_test_text_file:
                        fea_test_ori_list_temp = []
                        for i in range(args.test_text_n_sample_rounds):
                            if args.test_text_split:
                                fea_test_ori_ls_temp = random.sample(fea_test_ori_ls, len(fea_test_ori_ls))
                            else:
                                fea_test_ori_ls_temp = random.sample(fea_test_ori_ls[0], len(fea_test_ori_ls[0]))
                            fea_test_ori = ' '.join(paragraph for paragraph in fea_test_ori_ls_temp)
                            fea_test_ori_list_temp.append(fea_test_ori)
                        fea_test_ori_list.append(fea_test_ori_list_temp)

                    ## Get the relative test p_value
                    start_time = time.time()
                    power, p_value, t = two_sample_test(epoch, fea_test_ls=fea_test, test_flag=True, return_p_and_stat=True, relative_test_permutation=(args.relative_test_mode == 'permutation'))
                    end_time = time.time()
                ...
                if current_trial == args.trial_num:
                    ...
                    if args.relative_test:
                        ...
                        if len(set(answer_labels)) > 1:
                            baseline_power_hwt_list = [baseline_power_list[i] for i in range(len(baseline_power_list)) if answer_labels[i] == 0]
                            baseline_power_mgt_list = [baseline_power_list[i] for i in range(len(baseline_power_list)) if answer_labels[i] == 1]
                            print(f"The mmd-mp baseline power list for HWT is {baseline_power_hwt_list}!")
                            print(f"The mmd-mp baseline power list for MGT is {baseline_power_mgt_list}!")
                            print(f"The mmd-mp baseline avg_power for HWT: {avg_value(baseline_power_hwt_list)[0]} and std_power: {avg_value(baseline_power_hwt_list)[1]}")
                            print(f"The mmd-mp baseline avg_power for MGT: {avg_value(baseline_power_mgt_list)[0]} and std_power: {avg_value(baseline_power_mgt_list)[1]}")
                            print(f"The mmd-mp baseline auroc stats:")
                            process_p_values_and_labels_odd(answer_labels, baseline_p_value_list)
                            baseline_auroc_list, baseline_fpr_list, baseline_tpr_list = process_p_values_and_labels(answer_labels, baseline_p_value_list)
                            print(f"AUROC_LIST: {baseline_auroc_list}!")
                            # print(f"FPR_LIST: {baseline_fpr_list}!")
                            # print(f"TPR_LIST: {baseline_tpr_list}!")
                        else:
                            print(f"The mmd-mp baseline avg_power: {avg_value(baseline_power_list)[0]} and std_power: {avg_value(baseline_power_list)[1]}")
                        print(f"The mmd-mp baseline time list is {baseline_time_list}!")
                        print(f"The mmd-mp baseline total time is {np.sum(baseline_time_list)}!")
                        ...
                        if args.relative_test_baseline_simple:
                            ...
                            print(f"The simple relative test baseline t_stat list is {simple_relative_baseline_t_stat_list}!")
                            print(f"The simple relative test baseline power list is {simple_relative_baseline_power_list}!")
                            if len(set(answer_labels)) > 1:
                                simple_relative_baseline_power_hwt_list = [simple_relative_baseline_power_list[i] for i in range(len(simple_relative_baseline_power_list)) if answer_labels[i] == 0]
                                simple_relative_baseline_power_mgt_list = [simple_relative_baseline_power_list[i] for i in range(len(simple_relative_baseline_power_list)) if answer_labels[i] == 1]
                                print(f"The simple relative test baseline power list for HWT is {simple_relative_baseline_power_hwt_list}!")
                                print(f"The simple relative test baseline power list for MGT is {simple_relative_baseline_power_mgt_list}!")
                                print(f"The simple relative test baseline avg_power for HWT: {avg_value(simple_relative_baseline_power_hwt_list)[0]} and std_power: {avg_value(simple_relative_baseline_power_hwt_list)[1]}")
                                print(f"The simple relative test baseline avg_power for MGT: {avg_value(simple_relative_baseline_power_mgt_list)[0]} and std_power: {avg_value(simple_relative_baseline_power_mgt_list)[1]}")
                                print("The simple relative test baseline auroc stats:")
                                process_p_values_and_labels_odd(answer_labels, simple_relative_baseline_p_value_list)
                                simple_relative_baseline_auroc_list, simple_relative_baseline_fpr_list, simple_relative_baseline_tpr_list = process_p_values_and_labels(answer_labels, simple_relative_baseline_p_value_list)
                                ...
                                print(f"The simple relative test baseline avg_power: {avg_value(simple_relative_baseline_power_list)[0]} and std_power: {avg_value(simple_relative_baseline_power_list)[1]}")
                            print(f"The simple relative test baseline time list is {simple_relative_baseline_time_list}!")
                            print(f"The simple relative test baseline total time is {np.sum(simple_relative_baseline_time_list)}!")
                        ...
                        if args.relative_test_baseline:
                            ...
                            print(f"The relative test baseline t_stat list is {relative_baseline_t_stat_list}!")
                            print(f"The relative test baseline power list is {relative_baseline_power_list}!")
                            if len(set(answer_labels)) > 1:
                                relative_baseline_power_hwt_list = [relative_baseline_power_list[i] for i in range(len(relative_baseline_power_list)) if answer_labels[i] == 0]
                                relative_baseline_power_mgt_list = [relative_baseline_power_list[i] for i in range(len(relative_baseline_power_list)) if answer_labels[i] == 1]
                                print(f"The relative test baseline power list for HWT is {relative_baseline_power_hwt_list}!")
                                print(f"The relative test baseline power list for MGT is {relative_baseline_power_mgt_list}!")
                                print(f"The relative test baseline avg_power for HWT: {avg_value(relative_baseline_power_hwt_list)[0]} and std_power: {avg_value(relative_baseline_power_hwt_list)[1]}")
                                print(f"The relative test baseline avg_power for MGT: {avg_value(relative_baseline_power_mgt_list)[0]} and std_power: {avg_value(relative_baseline_power_mgt_list)[1]}")
                                print("The relative test baseline auroc stats:")
                                process_p_values_and_labels_odd(answer_labels, relative_baseline_p_value_list)
                                relative_baseline_auroc_list, relative_baseline_fpr_list, relative_baseline_tpr_list = process_p_values_and_labels(answer_labels, relative_baseline_p_value_list)
                                ...
                                print(f"The relative test baseline avg_power: {avg_value(relative_baseline_power_list)[0]} and std_power: {avg_value(relative_baseline_power_list)[1]}")
                            print(f"The relative test baseline time list is {relative_baseline_time_list}!")
                            print(f"The relative test baseline total time is {np.sum(relative_baseline_time_list)}!")


                        if fea_test_ori_list:
                            ## Output the test_ori_list into a txt file for manual checking
                            with open(f'./{PATH_exper}/HC3-{args.base_model_name}/{id}/test_data.txt', "w", encoding="utf-8") as file:
                                for i, lst in enumerate(fea_test_ori_list):
                                    file.write(f"=== Trial {i + 1} {'('+ ('MGT' if answer_labels[i] else 'HWT') +')' if answer_labels else args.test_dataset_answer} ===\n")
                            ...
                            with open(f'./{PATH_exper}/HC3-{args.base_model_name}/{id}/test_data.csv', "w", encoding="utf-8") as file:
                                writer = csv.writer(file)
                                writer.writerow(['trial', 'shuffle', 'label', 'text'])
                                for i, lst in enumerate(fea_test_ori_list):
                                    for j, paragraph in enumerate(lst):
                                        writer.writerow([i, j, 'MGT' if answer_labels[i] else 'HWT', repr(paragraph)])
                        ...
                        if args.binoculars:
                            ...
                            for i in range(len(fea_test_ori_list)):
                                ...
                                for j in range(len(fea_test_ori_list_temp)):
                                    ...
                                    end_time = time.time()
                                    result = "HWT" if "human" in bino.predict(fea_test_ori) else "MGT"
                                ...
                                binoculars_time_list.append(np.round(np.sum(binoculars_time_list_temp), 6))
                                binoculars_power_list.append(round(np.mean([p for p in binoculars_p_value_list_temp]), 6))
                                binoculars_p_value_list.append(binoculars_p_value_list_temp)
                                ## Take the most frequent result as the final result
                                binoculars_result_list.append(max(set(binoculars_result_list_temp), key=binoculars_result_list_temp.count))
                            ...
                            if len(set(answer_labels)) > 1:
                                binoculars_power_hwt_list = [binoculars_power_list[i] for i in range(len(binoculars_power_list)) if answer_labels[i] == 0]
                                binoculars_power_mgt_list = [binoculars_power_list[i] for i in range(len(binoculars_power_list)) if answer_labels[i] == 1]
                                print(f"The binoculars score list for HWT is {binoculars_power_hwt_list}!")
                                print(f"The binoculars score list for MGT is {binoculars_power_mgt_list}!")
                                print(f"The binoculars avg_score for HWT: {avg_value(binoculars_power_hwt_list)[0]} and std_score: {avg_value(binoculars_power_hwt_list)[1]}")
                                print(f"The binoculars avg_score for MGT: {avg_value(binoculars_power_mgt_list)[0]} and std_score: {avg_value(binoculars_power_mgt_list)[1]}")
                                process_p_values_and_labels_odd(answer_labels, binoculars_power_list)
                                binoculars_auroc_list, binoculars_fpr_list, binoculars_tpr_list = process_p_values_and_labels(answer_labels, binoculars_power_list)
                                print(f"AUROC_LIST: {binoculars_auroc_list}!")
                                # print(f"FPR_LIST: {binoculars_fpr_list}!")
                                # print(f"TPR_LIST: {binoculars_tpr_list}!")
                            else:
                                print(f"The binoculars avg_score: {avg_value(binoculars_power_list)[0]} and std_score: {avg_value(binoculars_power_list)[1]}")
                            print(f"The binoculars time list is {binoculars_time_list}!")
                            print(f"The binoculars total time is {np.sum(binoculars_time_list)}!")
                        ...
                        if len(set(answer_labels)) > 1:
                            relative_test_power_hwt_list = [relative_test_power_list[i] for i in range(len(relative_test_power_list)) if answer_labels[i] == 0]
                            relative_test_power_mgt_list = [relative_test_power_list[i] for i in range(len(relative_test_power_list)) if answer_labels[i] == 1]
                            print(f"The relative test power list for HWT is {relative_test_power_hwt_list}!")
                            print(f"The relative test power list for MGT is {relative_test_power_mgt_list}!")
                            print(f"The relative test avg_power for HWT: {avg_value(relative_test_power_hwt_list)[0]} and std_power: {avg_value(relative_test_power_hwt_list)[1]}")
                            print(f"The relative test avg_power for MGT: {avg_value(relative_test_power_mgt_list)[0]} and std_power: {avg_value(relative_test_power_mgt_list)[1]}")
                            print("The relative test auroc stats:")
                            process_p_values_and_labels_odd(answer_labels, relative_test_p_value_list)
                            relative_test_auroc_list, relative_test_fpr_list, relative_test_tpr_list = process_p_values_and_labels(answer_labels, relative_test_p_value_list)
                            ...
                            print(f"The relative test avg_power: {avg_value(relative_test_power_list)[0]} and std_power: {avg_value(relative_test_power_list)[1]}")
                        print(f"The relative test time list is {relative_test_time_list}!")
                        print(f"The relative test total time is {np.sum(relative_test_time_list)}!")
