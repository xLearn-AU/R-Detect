...
def load_HC3(data_o):
    ...
    long_train_real = [
        x for x, y in zip(long_train_real, tokenized_data["input_ids"]) if len(y) <= 512
    ]
    ...
    for seed in range(990, 990 + args.trial_num):
            ...
            for text_generated_model_name in args.text_generated_model_name:
                ...
                if args.two_sample_test:
                    ...
                    generated_sent_token = [
                        nltk.sent_tokenize(text)[1:-1] for text in generated
                    ]
                    ...
                    real_data_temp_seletced = [
                        pa_ls for pa_ls in real_data_temp if len(pa_ls) >= 5
                    ]
                    generated_data_temp_seletced = [
                        pa_ls for pa_ls in generated_data_temp if len(pa_ls) >= 5
                    ]
                    len_data = min(
                        len(real_data_temp_seletced), len(generated_data_temp_seletced)
                    )
        ...
        def two_sample_test(
            ...
            with torch.no_grad():
                ...
                generated_test_power_ls = mmd_two_sampe(
        ...
        for i in range(num_target):
            power = two_sample_test(
                ...
                fea_generated_ls=fea_generated[i * test_lenth : (i + 1) * test_lenth],
                test_flag=True,
                N=10,
            )
        ...
        print(
            f"avg_power: {avg_auroc(all_power_list)[0]} and std_power: {avg_auroc(all_power_list)[1]}"
        )
        print(
            f"avg_auroc: {avg_auroc(all_aruoc_list)[0]} and std_auroc: {avg_auroc(all_aruoc_list)[1]}"
        )
